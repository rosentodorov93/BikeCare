...
vi.mock('../repositories/bicycles.repository.js', () => ({
  bicycleRepository: { findById: vi.fn() },
}));
...
describe('componentService.getByBikeId', () => {
  it('throws ApiError(404) when the bike does not exist', async () => {
    ...
    await expect(componentService.getByBikeId('ghost')).rejects.toMatchObject({
      status: 404,
  ...
  it('returns components with computed wearState', async () => {
    vi.mocked(bicycleRepository.findById).mockReturnValue(sampleBike); // totalDistance: 1200
    vi.mocked(componentRepository.findByBikeId).mockReturnValue([
      { ...sampleComponent, serviceIntervalKm: 3000, distanceAtService: 0 },
    ]);

    const result = await componentService.getByBikeId('bike-1');
    // 1200 / 3000 * 100 = 40%
    expect(result[0].wearState).toBe(40);
  });

  it('returns wearState > 100 for overdue components', async () => {
    vi.mocked(bicycleRepository.findById).mockReturnValue({ ...sampleBike, totalDistance: 5000 });
    vi.mocked(componentRepository.findByBikeId).mockReturnValue([
      { ...sampleComponent, serviceIntervalKm: 3000, distanceAtService: 0 },
    ]);
...
describe('componentService.resetService', () => {
  it('throws ApiError(404) when the bike does not exist', async () => {
    vi.mocked(bicycleRepository.findById).mockReturnValue(undefined);
    await expect(componentService.resetService('ghost', 'comp-1')).rejects.toMatchObject({
      status: 404,
      code: 'BICYCLE_NOT_FOUND',
    });
  });
  ...
  it('throws ApiError(404) when the component does not exist', async () => {
    ...
    await expect(componentService.resetService('bike-1', 'missing')).rejects.toMatchObject({
      status: 404,
  ...
  it('throws ApiError(404) when the component belongs to a different bike', async () => {
    ...
    await expect(componentService.resetService('bike-1', 'comp-1')).rejects.toMatchObject({
      status: 404,
      code: 'COMPONENT_NOT_FOUND',
    });
  });
  ...
  it('resets distanceAtService to the bike current total and returns wearState 0', async () => {
    ...
    const result = await componentService.resetService('bike-1', 'comp-1');
  ...
  it('calls setDistanceAtService with the bike total distance', async () => {
    ...
    await componentService.resetService('bike-1', 'comp-1');
