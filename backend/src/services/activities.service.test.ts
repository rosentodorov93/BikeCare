...
describe('activityService.getAll', () => {
  it('delegates to the repository', async () => {
    vi.mocked(activityRepository.findAll).mockReturnValue([sampleActivity]);
    const result = await activityService.getAll();
    expect(activityRepository.findAll).toHaveBeenCalledOnce();
...
describe('activityService.create', () => {
  it('throws ApiError(404) when the bike does not exist', async () => {
    ...
    await expect(activityService.create(createDto)).rejects.toMatchObject({
      status: 404,
  ...
  it('throws ApiError referencing the missing bike id', async () => {
    vi.mocked(bicycleRepository.findById).mockReturnValue(undefined);
    await expect(activityService.create({ ...createDto, bikeId: 'ghost-bike' })).rejects.toThrow(
      ApiError,
    );
  });
  ...
  it('inserts the activity and bumps bike distance in a transaction', async () => {
    ...
    await activityService.create(createDto);
  ...
  it('returns the persisted activity from findById', async () => {
    ...
    const result = await activityService.create(createDto);
